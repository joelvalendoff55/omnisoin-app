-- Fix the audit trigger to use NULL when no user is authenticated
...
DECLARE
  ...
  v_user_id UUID;
BEGIN
  -- Get authenticated user or NULL for admin operations
  v_user_id := auth.uid();
  ...
  IF v_structure_id IS NOT NULL THEN
    INSERT INTO public.immutable_audit_log (
      ...
      v_event_type,
      v_user_id, -- NULL if not authenticated
      v_structure_id,
      TG_TABLE_NAME,
      CASE 
      ...
      v_new_value,
      '0.0.0.0'::inet,
      encode(extensions.digest((COALESCE(v_previous_value::text, '') || COALESCE(v_new_value::text, '') || now()::text)::bytea, 'sha256'), 'hex')
    );
  END IF;
